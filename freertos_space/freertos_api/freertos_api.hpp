...
class rtos_ui
{
	...
	rtos_ui() :pwm_freq_mutex{xSemaphoreCreateMutex()},
			...
			BaseType_t lock(SemaphoreHandle_t mutex_to_lock) {
				return xSemaphoreTake(mutex_to_lock, portMAX_DELAY);
			}

			BaseType_t unlock(SemaphoreHandle_t mutex_to_lock) {
				return xSemaphoreGive(mutex_to_lock);
			}

			void set_pwm_freq(uint32_t tmp_pwm_freq) {
				if(lock(pwm_freq_mutex)){
